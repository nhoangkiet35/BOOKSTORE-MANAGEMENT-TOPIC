...
from bookstore import db, utils
...
class AuthenticatedView(BaseView):
    def is_accessible(self):
        return (
            ...
            and current_user.has_role("staff")
        )
...
class ConfigurationView(ModelView):
    ...
    def update_model(self, form, model):
        if form.validate():
            try:
                if (
                    int(form.min_import_quantity.data) > 0
                    and int(form.min_stock_quantity.data) > 0
                    and int(form.time_to_end_order.data) > 0
                    and int(form.quick_ship.data) >= 0
                ):
