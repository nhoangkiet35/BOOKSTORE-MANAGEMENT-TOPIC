...
class Order(db.Model):
    ...
    def order_total(self):
        return (
            db.session.quey(db.func.sum(OrderDetails.quantity * Book.unit_price))
            .join(Book)
            .filter(OrderDetails.order_id == self.id)
            .scalar()
            + 1000
        )
    ...
    def quantity_total(self):
        return (
            db.session.query(db.func.sum(OrderDetails.quantity))
            .filter(OrderDetails.order_id == self.id)
            .scalar()
        )
