...
from bookstore.models import (Author, Book, Category, Comment, Configuration,
                              PaymentMethod, Role, User)
    ...
    with app.app_context():
        ...
        for customer in customer_list:
            ...
            order = utils.create_order(
                customer.id, staff_id, order_details, in_cash.id, initial_date
            )
            rand_num = random.randint(1, 10)
            utils.order_paid_incash(
                order.total_payment,
                order.id,
                order.initiated_date + datetime.timedelta(hours=rand_num),
            )
            utils.order_delivered(
                order.id, order.initiated_date + datetime.timedelta(hours=rand_num + 1)
            )
