...
class VerifyAccountForm(FlaskForm):
    first_char = StringField(
        "First", validators=[DataRequired()], render_kw={"maxlength": 1}
    )
    second_char = StringField(
        "Second", validators=[DataRequired()], render_kw={"maxlength": 1}
    )
    third_char = StringField(
        "Third", validators=[DataRequired()], render_kw={"maxlength": 1}
    )
    fourth_char = StringField(
        "Fourth", validators=[DataRequired()], render_kw={"maxlength": 1}
    )
    fifth_char = StringField(
        "Fifth", validators=[DataRequired()], render_kw={"maxlength": 1}
    )
    submit = SubmitField("Verify")

    def to_code(self):
        return (
            self.fifth_char.data
            + self.second_char.data
            + self.third_char.data
            + self.fourth_char.data
            + self.fifth_char.data
        )
