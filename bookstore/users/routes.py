...
from flask_security import (current_user, login_required, logout_user,
                            roles_accepted)
from flask_security.utils import hash_password, login_user, verify_password

from bookstore import dao, db, user_datastore
...
from bookstore.users.forms import (LoginForm, RegistrationForm,
                                   UpdateAccountForm, VerifyAccountForm)
from bookstore.users.utils import (extract_search_user_by_phone,
                                   resend_register_code, save_picture,
                                   send_verify_code, verify_account)
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            if send_verify_code(user_id=new_user.id) == 0:
                flash(
                    f"Your account has been created! Please confirm your account by the code we sent to your email!",
                    "success",
                )
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user.active:
            flash(
                "Please verify your account by the code we sent to your email", "danger"
            )
...
def staff():
    ...
    if form.validate_on_submit() and id is not None and request.method == "POST":
        ...
        index = [
            index for index, prod in enumerate(session["cart"]) if prod["id"] == int(id)
        ]
        if len(product) == 0:
